/*
BetterLog code copied from 'https://github.com/peterherrmann/BetterLog'
Preferred not to import the library and keep the project standalone.


Extends default google apps script logging.
...
var BetterLog = function(useAdvLog){
  ...
  this.useSpreadsheet = function(optKey, optSheetName){
    if(useAdvLog == true){
      return useSpreadsheet(optKey, optSheetName)
    }
    return this;
  };
  
  this.log = function(message, optValues){
    if(useAdvLog == true){
      return log(message, optValues)
    }
    return nativeLogger_.log(message)
  };
  
  this.info = function(message, optValues){
    if(useAdvLog == true){
      return info(message, optValues)
    }
    return nativeLogger_.log(message)
  };
  
  this.severe = function(message, optValues){
    if(useAdvLog == true){
      return severe(message, optValues)
    }
    return nativeLogger_.log(message)
  };
  
  this.warning = function(message, optValues){
    if(useAdvLog == true){
      return warning(message, optValues)
    }
    return nativeLogger_.log(message)
  };
